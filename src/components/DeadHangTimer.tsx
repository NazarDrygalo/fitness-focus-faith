...
export function DeadHangTimer({ onFinish, disabled = false, initialSeconds = 0 }: DeadHangTimerProps) {
  ...
  if (finished) {
    ...
    return (
      <div className="text-center p-4 rounded-lg bg-success/15 text-success">
        <p className="text-sm font-medium">✅ Dead hang completed: {Math.floor(secs / 60)}m {secs % 60}s</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex gap-1 p-1 bg-secondary rounded-lg w-fit mx-auto">
        {(["stopwatch", "timer"] as Mode[]).map((m) => (
          ...
          >
            {mode === m && (
              <motion.div layoutId="deadhang-mode" className="absolute inset-0 bg-accent rounded-md" transition={{ type: "spring", stiffness: 400, damping: 30 }} />
            )}
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          {laps.length > 0 && (
            <div className="w-full space-y-1 max-h-32 overflow-y-auto">
              {laps.map((lap, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex justify-between text-sm px-3 py-1.5 bg-secondary rounded-md">
          ...
          {!timerStarted ? (
            ...
            <div className="relative flex items-center justify-center">
              <svg width="180" height="180" className="-rotate-90">
                ...
                <motion.circle cx="90" cy="90" r={circleRadius} fill="none" stroke="hsl(var(--primary))" strokeWidth="8" strokeLinecap="round" strokeDasharray={circumference} animate={{ strokeDashoffset }} transition={{ duration: 0.1 }} />
              </svg>
              <p className="absolute text-3xl font-mono font-bold text-foreground tabular-nums">{formatTime(timerRemaining)}</p>
            </div>
          )}
          <div className="flex gap-2">
            {!timerStarted ? (
